...
class Maze:
    def __init__(self, mazefile):
        ...
        self._land_surf = pygame.transform.scale(
            self._land_surf, (self.tile_size_x, self.tile_size_y)
        )
        self._land_surf_start = pygame.image.load("assets/Images/land_start.png")
        self._land_surf_start = pygame.transform.scale(
            self._land_surf_start, (self.tile_size_x, self.tile_size_y)
        )
        self._land_surf_finish = pygame.image.load("assets/Images/land_finish.png")
        self._land_surf_finish = pygame.transform.scale(
            self._land_surf_finish, (self.tile_size_x, self.tile_size_y)
        )
        self._wall_surf = pygame.image.load("assets/Images/wall_9.png")
        self._wall_surf = pygame.transform.scale(
            self._wall_surf, (self.tile_size_x, self.tile_size_y)
        )
        ...
        self._obstacle_surf = pygame.transform.scale(
            self._obstacle_surf,
            (1.3 * ITEM_SIZE * self.tile_size_x, 1.3 * ITEM_SIZE * self.tile_size_y),
        )
        self._door_surf = pygame.image.load("assets/Images/door.png")
        self._door_surf = pygame.transform.scale(
            self._door_surf, (self.tile_size_x, self.tile_size_y)
        )
        ...
        for i in range(len(self.maze)):
            for j in range(len(self.maze[i])):
                if self.maze[i][j] == "1":
                    ...
                    self.start = [
                        (j + 0.2) * self.tile_size_x,
                        (i + 0.2) * self.tile_size_y,
                    ]
                    cell = pygame.Rect(
                        (j * self.tile_size_x, i * self.tile_size_y),
            ...
            for j in range(len(self.maze[i])):
                if self.maze[i][j] == COIN:
                    ...
                    new_treasure = pygame.Rect(
                        (j * self.tile_size_x, i * self.tile_size_y),
                        (
                            4 * ITEM_SIZE * self.tile_size_x,
                            3 * ITEM_SIZE * self.tile_size_y,
                        ),
                    )
    ...
    def draw(self, display_surf):
        thinwall_surf = pygame.transform.scale(
            self._wall_surf, (self.tile_size_x, 0.1 * self.tile_size_y)
        )
        for i in range(len(self.maze)):
            for j in range(len(self.maze[i])):
                if self.maze[i][j] == "1":
                    display_surf.blit(
                        self._wall_surf, (j * self.tile_size_x, i * self.tile_size_y)
                    )
                elif self.maze[i][j] == "S":
                    display_surf.blit(
                        self._land_surf_start,
                        (j * self.tile_size_x, i * self.tile_size_y),
                    )
                    display_surf.blit(
                        thinwall_surf, (j * self.tile_size_x, i * self.tile_size_y)
                    )
                elif self.maze[i][j] == "E":
                    display_surf.blit(
                        self._land_surf_finish,
                        (j * self.tile_size_x, i * self.tile_size_y),
                    )
                    display_surf.blit(
                        thinwall_surf,
                        (j * self.tile_size_x, (i + 0.9) * self.tile_size_y),
                    )
                else:
                    display_surf.blit(
                        self._land_surf, (j * self.tile_size_x, i * self.tile_size_y)
                    )
